...
-- 식품정보와 제조사정보를 JOIN하여 데이터가 정상적으로 임포트되었는지 확인
-- 여기에서 데이터가  출력된다면 제조사정보 테이블에 문제가 있는 것이다
SELECT *
FROM tbl_foods F
    LEFT JOIN tbl_company C
        ON F.fd_ccode = C.cp_code
    WHERE C.cp_code IS NULL; -- 제조사정보에 NULL 값이 있는지 확인
...
-- FK 설정은 자손 talbe에 설정하기

ALTER TABLE tbl_foods
ADD CONSTRAINT fk_company -- 제약조건 설정(fk_company 이름으로 설정)
