...
SELECT DECODE(in_inout, '1', '매입', '매출') AS 구분,
    COUNT(*)
FROM tbl_iolist
GROUP BY DECODE(in_inout, '1', '매입', '매출');

-- IF io_inout == 1 THEN '매입'
-- ELSE IF io_inout == 2 THEN '매출'
SELECT DECODE(in_inout, '1', '매입', '2', '매출') AS 구분,
    COUNT(*)
FROM tbl_iolist
GROUP BY DECODE(in_inout, '1', '매입', '2', '매출');

/*
매입매출 데이터를 DB import 한후
 테이블에서 상품저보와 거래처 정보를 분리하여
 제 3정규화를 수행하기
 
현재 매입매출 테이블에 상품이름과 거래처명(대표포함)이 저장되어 있다
 현재 테이블에서 만약 상품이름이나 거래처명이 변경되어야 하는 일이 발생한다면
 다수의 데이터(레코드)에 변경(update)가 되는 상황이 만들어진다
 다수의 데이터를 변경하는 명령은 데이터 무결성을 해치는 원인중의 하나이다
 
상품테이블을 별도로 분리하고, 
 상품코드, 상품명 형식으로 저장한 후  
 매입 매출 테이블에는 상품명 대신 상품코드를 포함하고 
 이후에는 JOIN을 통해 데이터를 조회하는 것이 좋다
...
매입 단가가 0인경우
 ...
 매출단가는 매입단가에 20% 마진을 추가, 부가세 10% 추가
 매출단가 = (매입단가 * 1.2) * 1.1
