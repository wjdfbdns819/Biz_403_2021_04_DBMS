...
-- DBMS 에서는 char, varchar 타입의 문자열 데이터도
-- 범위를 지정하여 조회할수 있다
--  단, 모든 데이터의 길이가 같을때
SELECT * 
FROM tbl_student
...
-- LIKE는 조회 연산자 중에서 가장 느림
SELECT *
FROM tbl_student
WHERE st_name LIKE '기%';

-- Full Scan 검색
-- LIKE 조회 연산자 중에서도 가장 느림
...
FROM tbl_student
WHERE st_adder LIKE '%북%'
ORDER BY st_adder; -- 자동적으로 ASC(오름차순) 가나다 순, ABC 순, 12345 순

SELECT *
FROM tbl_student
WHERE st_adder LIKE '%북%'
ORDER BY st_adder DESC; -- DESCENDING(내림차순) 다나가, CBA, 54321
