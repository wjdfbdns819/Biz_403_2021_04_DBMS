...
-- Sub Query
-- SQL 명령문 내에 또다른 SQL 명령문을 포함하는 코드임
-- JOIN 대신 사용하는 코드이고,
--  Sub Query 99%정도는 JOIN을 사용하여 대체할수 있다
SELECT sc_num AS 학번,
    (SELECT st_name 
    FROM tbl_student
    WHERE sc_num = st_num
...
/*
 tbl_score 테이블은 학번, 국어, 영어, 수학 칼럼으로 구성되어있다
  ...
  기존에 사용하던 table을 변경해야하는 상황이 발생한다
  테이블을 변경하는 것은 많은 데이터가 담겨있는 경우
  매우 위험한 실행이 될 수 있다
  테이블이 변경되는 동안에는 테이블에 Lock이 걸리고
  그 동안은 해당 테이블에 어떤한 쿼리도 적용할수 없다
  경우데 따라 기존 데이터가  손상되는 경우도 있다
...
-- 학생 점수를 저장할 테이블을  정규화 하기 위해
--  재설계를 하였다
--  칼럼으로 되어 있던 과목을 데이터화 하여
--  학번, 과목명 형식으로 입력하도록 설계하였다
--  이렇게 설계를 하고 나니 학번, 과목명 어떤 칼럼도 단독으로 PK로 
--  설정할수 없게 되었다
--  테이블에 PK가 없으면
...
--   단독 칼럼으로 만들수 없는 경우는 2개 이상의 칼럼을 묶어서
--   만들어야 하는 경우도 있다

-- 이런 상황에 직면하면 실제 데이터에는 없는 
--  칼럼을 하나 추가하고 별도로 PK로 설정한다
CREATE TABLE tbl_scoreV2(
...
-- 삭제나 갱신을 할때는 WHERE 조건문에 PK를 대상으로
--  실행하는것이 좋은데, 아래와 같은 코드를 사용하면
--  잦은 실수를 반복할 수 있다
