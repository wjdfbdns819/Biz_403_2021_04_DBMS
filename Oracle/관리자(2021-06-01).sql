...
오라클에서는 관리자(sys,system,sysdba) 계정이 존재하고,
 ...
 데이터 무결성 파괴 : 허가된 사용자에게 권한이 잘못 부여되거나,
                        과도하게 권한을 부여하여 명령을 잘못사용하여
                        데이터에 문제를 일으키는 것
                        
 관리자 권한에서는 최소한으로 테이블스페이스 생성, 사용자 생성,
  사용자에게 권한부여하는 정도만 사용하도록 권장한다
...
testUser라는 '사용자계정(User Account)'을 생성하고,
 비번을 testUser로 하고, 기본 TableSpace는 testDB로 설정
*/
-- 사용자 생성
...
/*
최초로 사용자계정을 생성한 후에는 아직 아무런 권한이 없기때문에
 DB에 접속하는 것조차 할 수 없다
 실습을 쉽게 하기 위하여 생성한 사용자 계정에 DBA 권한을 부여한다
 
 DBA 권한 : 시스템 관련 DB에 접근은 할수 없으나
             기타 table을 만드는 일부터 대부분의 DBMS 관련 명령을 사용하여
             DB를 핸들링 할수 있는 권한이다
*/

-- testUser에세 DBA 권한 부여
