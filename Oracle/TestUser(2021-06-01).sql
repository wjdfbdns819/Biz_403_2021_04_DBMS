...
SELECT bk_comp AS 출판사코드, COUNT(*)
        
FROM tbl_books
GROUP BY bk_comp;

SELECT bk_comp AS 출판사코드, cp_name AS 출판사명, COUNT(*) AS 권수
FROM tbl_books B
...
SELECT SUM(bk_price) AS 전체합계금액
FROM tbl_books
WHERE bk_price >= 20000;

-- tbl_books, tbl_company, tbl_author 세개의 table을 JOIN하여
-- ISBN, 도서명, 출판사명, 출판사대표, 저자, 저자 연락처가 보이도록
-- SQL 작성

SELECT BK.bk_isbn AS ISBN,
        BK.bk_title AS 도서명,
        CP.cp_name AS 출판사명,
        CP.cp_ceo AS 출판사대표,
        AU.au_name AS 저자,
...
FROM tbl_books BK
    LEFT JOIN tbl_company CP
        ON BK.bk_comp = CP.cp_code
    LEFT JOIN tbl_author AU
        ON BK.bk_author = AU.au_code
    ORDER BY bk_isbn;
    
-- tbl_books, tbl_company, tbl_author 세개의 table을 JOIN하여
-- ISBN, 도서명, 출판사명, 출판사대표, 
-- 저자, 저자 연락처, 출판일이 보이도록 SQL 작성
-- 단 출판일이 2018년인 데이터만
...
REFERENCES tbl_company(cp_code); -- 참조 대상
