...
-- TABLE
--  DBMS를 사용하여 데이터를 추가하면 마치 excel의 sheet에 데이터를
--   저장하는 것처럼 데이터가 저장된다
--  실제 DBMS 소프트웨어가 저장하는 방식은 고유한 기술이 적용되어 있지만
--   DBMS 소프트웨어를 사용하는 사용자는 모든 데이터를 표(TABLE)형식으로
...
--              최대 크기보다 짧은 데이터를 저장하면 자동으로 공간을 
--               축소하여 데이터를 저장한다
--                "가변길이 문자열"이라고 부름

--  nVARCHAR : 한글과 같은 UNICODE 문자열을 저장할때
--              영문 알파벳과 저장하는 방식이 달라서
--              문제를 일으킬수 있다
...
--   오라클에서는 NUMBER 키워드로 일괄 지정하여도 상관없다
--   저장되는 자릿수에 따라 자동으로 크기가 조절된다


CREATE TABLE tbl_score (
    sc_num CHAR(5),  --> 문자열 5자리의 데이터만 저장함
                        --(Java 에서는 String num 코드임)
                        
    sc_name nVARCHAR2(20), --> 문자열 20자리까지 데이터 저장(가변적임)
                        --(Java 에서는 String name)
    sc_kor NUMBER,
    sc_eng NUMBER,
    sc_math NUMBER
);

-- DML(Date Manipulation Lang., Data Management Lang. 데이터 조작어, 데이터 관리)
-- 생성된 TABLE에 데이터 추가, 조회, 수정, 삭제를 하는 명령어

-- DBMS 대표적인 CRUD 업무
--  데이터 추가(데이터 생성 : create 업무) : INSERT 명령어 사용
--  데이터 조회(읽기 등... : Read) : SELECT 명령어 사용
--  데이터 변경, 수정 : UPDATE 명령어 사용
--  데이터 삭제 : DELETE
...
INSERT INTO tbl_score(sc_name) VALUES('이몽룡'); -->  // 데이터 추가 연습

-- 데이터 읽기
SELECT sc_num, sc_kor, sc_eng, sc_math
FROM tbl_score;

SELECT sc_name FROM tbl_score; --> // 데이터 읽기 연습
