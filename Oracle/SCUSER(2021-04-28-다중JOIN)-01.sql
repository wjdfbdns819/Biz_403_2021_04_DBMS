...
-- 여러 개의 데이터를 동시에 INSERT하기
--  다른 테이블로부터 데이터를 복사할때 사용하는 방식
INSERT ALL
...
CREATE TABLE tbl_student(
    -- 제약조건 설정
    st_num	CHAR(5)	PRIMARY KEY, -- 학번은 기본키
...
ORDER BY 학번; -- 학번순으로 정렬하여 조회

-- 학생정보 테이블에서 학과별로 몇명의 학생이 재학중인지
-- 학과코드 = 학과명은 항상 같은값이 되므로
--  학과코드, 학과명으로 GROP BY를 하면
--  학과별로 묶음이 이루어진다
...
CRATE VIEW view_성적일람표 AS
(
    SELECT SC.sc_num 학번, 
            ST.st_name 이름, ST.st_dcode 학과코드, DP.dp_prof 담당교수,
            ST.st_tel 전화번호,
...
-- 2.
SELECT AVG(평균) -- ROUND(AVG(평균),0) 사용하여 반올림 해도 됨
FROM view_성적일람표
WHERE 평균 >= 75;
