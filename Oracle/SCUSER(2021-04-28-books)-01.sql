...
-- ALTER TABLE : TABLE을 변경하는 명령 (DDL 명령어)
...
/*
도서 정보를 저장하기 위하여 tbl_books 테이블을 생성하고,
 도서정보를 import 했다
도서 정보는 어플로 만들기저에 사용하던 데이터인 관계로
...
데이터를 저장할 칼럼을 크게 설정하여 입력(import)하는데는
 문제가 없는데 저자나 역자를 기준으로 데이터를 여러가지 조건을 부여하여
 조회를 하려고 하면 문제가 발생할 수 있다
 특히 저자 이름으로 GROUPPING을 하여 데이터를 조회해 보려고 하면
 상당히 어려움을 겪을수 있다 
*/
...
RENAME COLUMN bk_author TO bk_author1; ---bk author 칼럼을 bk_author 칼럼으로 변경

-- bk_author2라는 칼럼을 생성
-- 한글 가변문자열 2로 선언하고 NOT NULL로 설정하라
-- ALTER TABLE을 이용하여 칼럼을 추가하는 경우에는
--  사전에 제약조건 설정이 매우 까다롭다
-- 제약조건을 설정하려면
-- 1. 칼럼을 아루런 제약조건없이 추가한후
...
데이터 베이스의 제1 정규화
 ...
 이미 2개 이상의 데이터가 저장된 경우 분리하여 원자성을 갖도록하는 것이다
 */

/*
도서 정보 데이터의 제1 정규화를 수행하고 보니
 저자 데이터를 저장할 칼럼이 이후에 또 변경해야하는 상황이 발생할수 있는 
 이슈가 발견되었다

제2 졍규화를 통하여 테이블 설계를 다시 해야 하겠다
 1. 정규화를 수행할 칼럼이 무엇인가 파악(인식한다)
     저자 데이터를 저장할 칼럼
     복수의 데이터가 필요한 경우
     
 2. 도서 정보와 관련된 저자 데이터를 저장할 Table을 새로 생성한다
...
-- 제 1정규화가 완료된 데이터로 다시 import
DELETE FROM tbl_books;
COMMIT;
SELECT * FROM tbl_books;

-- 제 1 정규화가 완료된 도서 정보로 부터 저자리스트를 생성

-- 도서 정보 테이블에서 저자이름들 목록을 추출하기
...
WHERE bk_author2 IS NOT NULL -- bk_author2 칼럼에 데이터가 있는 경우만 조회
...
-- 저자정보에는 ISBN과 저자명을 포함한 데이터를 추가 해주면된다
...
-- 9791188850440 이 도서에 저자를 추가하고 싶을때
-- 테이블의 ba_seq 칼럼에는 이미 등록된 값이 아닌
-- 새로운 숫자를 사용하여 데이터를 추가해야 한다
-- 데이터를 추가할때마다 새로운 값이 무엇인지 알아야 하는 매우 불편한
--  상황이 만들어지고 말았다
INSERT INTO tbl_book_author(be_seq, ba_isbn, ba_author)
VALUES( 35, '9791188850440', '홍길동');
