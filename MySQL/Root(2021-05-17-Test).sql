-- 현재 존재하는  DateBase를  보여달라
show databases;

-- 지금부터 MYSQL DateBase를 사용하겠다 
-- 사용자와 연관 없이 기본적으로 사용할 
-- DateBase를 지정하여 사용할 준비 
USE MYSQL;

-- 현재 접속한 DateBase(mysql)에 있는 모든 table을 보여달라 
SHOW tables;
 
-- myDB라는 DateBase(데이터 저장소) 생성 
...
CREATE TABLE tbl_test(
	...
	id BIGINT PRIMARY KEY AUTO_INCREMENT, -- id 값이 insert 될때마다 자동으로 증가 
...
-- 명령이 정상으로 수행되면 아무런 메시지도 보이지 않는다 
...
-- 도서 가격이 20000원 이상인 데이터 조회
SELECT * 
FROM tbl_books
WHERE bk_price >= 20000;

-- 도서 가격이 만원 이상 2만원 이하인 데이터 조회
SELECT *
FROM tbl_books
...
-- 도서명에 '왕' 문자열이 있는 데이터 조회
SELECT *
FROM tbl_books
WHERE bk_title LIKE '%왕%';

-- Java 등 코딩에서 중간 문자열 검색
-- Oracle 에서는 '%' || 왕 || '%' 을 사용하여 검색
-- MySQL 에서는 CONCAT('%', '왕', '%')
SELECT *
FROM tbl_books
...
-- 4(0부터 시작하여 3번)번째 데이터부터 2개를 보여라
-- 게시판 등 코딩에서 pagination을 구현할때 사용하는 방법
SELECT *
FROM tbl_books
LIMIT 3, 2;
