...
-- Tabel 생성 도서대여 프로젝트의 회원 정보 Table
...
INSERT INTO tbl_buyer(bu_code, bu_name, bu_birth)
VALUES (1, 'ooo', 0000);

INSERT INTO tbl_buyer(bu_code, bu_name, bu_birth)
VALUES (1, 'ooo', 0000);

-- tbl_book_rent Table 생성
...
-- 기존의 사용하던 SEQ를 삭제하고 다시 생성할 경우
--  반드시 적용되는 Table의 칼럼 값을 확인해야 한다
--  seq 칼럼의 최대값을 확인하고 
SELECT MAX(br_seq) FROM tbl_book_rent;

-- SEQ의 START 값을 적용하는 테이블의 SEQ 최대값보다 크게 설정을 해야함
CREATE SEQUENCE seq_book_rent
START WITH 11
INCREMENT BY 1;

INSERT INTO tbl_book_rent(br_seq, br_sdate, br_isbn, br_bcode)
VALUES (seq_book_rent.NEXTVAL,  '2021-04-20', '9791162540770', 'B0020');

SELECT * FROM tbl_book_rent;

-- tbl_book_rent는 회원이 도서를 대여한 리스트가 있다
--  여기에는 도서코드, 회원코드만 있기때문에
--  구체적인 정보를 알수 없다
...
-- 4월 25일 이전에 대여했는데 아직 반납하지 않은 사람 조회
SELECT * FROM view_도서대여정보
...
-- 전체 데이터 중 조건에 맞는 데이터를 가져와서 그룹으로 묶음
...
-- 급여 테이블에서 영업직을 제외한 직업의 급여 총합계를 계산하고,
...
VALUES (seq_book_rent.NEXTVAL, '2021-04-01', 'B0012', '9791188850549');


INSERT INTO tbl_book_rent(br_seq,
            br_sdate, br_bcode, br_isbn)
VALUES (seq_book_rent.NEXTVAL, '2021-04-01', 'B0011', '9791162540756');
