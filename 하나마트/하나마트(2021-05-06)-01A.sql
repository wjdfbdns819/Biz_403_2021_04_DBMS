...
-- 일년동안 매입과 매출금액을 계산하고, 단순 이익금 계산해 보기
SELECT 
    SUM(DECODE(io_inout, '매입', io_total)) AS 매입합계,
    SUM(DECODE(io_inout, '매출', io_total)) AS 매출합계,
...
--  5만원 정도의 금액에 판매하게 된다
--  쌀을 공장에서 가공을 하여 생산품(공산품)으로 만들게 되면
...
SELECT io_dname, 
    SUM(DECODE(io_inout, '매입', io_total, 0)) AS 매입합계, -- 값이 없으면 null 아닌 0으로 표시
...
-- 상품리스트와 매입 매출 단가를 조회하기
-- 같은 상품이라도 거래 시기에 따라 매입과 매출 금액이 
--  달라질수 있기때문에 데이터중에서 제일 높은 단가를 가져오기
SELECT io_pname,
...
2. 매입, 매출 구분에 따라 각각 매입 단가, 매출단가를 가져오기
3. 매입과 매출에 0인 값이 있다

4. 매입 단가가 0인 데이터는 매출데이터에서 임의로 생성하기
    매출단가의 80%를 매입단가로 하고, 부가세를 제외한 금액으로 계산
    E2 항목의 값에 0.8을 곱하여 E2의 80% 가격이 되고,
...
/*
매입매출 데이터로 부터 상품정보, 거래처 정보 데이터를 생성하고
테이블을 생성하여 데이터를 import 했다

매입 매출데이터와, 상품정보, 거래처 정보를 
JOIN하기 위해서는 매입매출 데이터에 상품코드, 거래처 코드가 있어야 한다
 그러나 현재 데이터는 코드 칼럼이 없이 이름 칼럼만 있는 상태이다
 
매입 매출 데이터에 상품코드, 거래처코드 칼럼을 추가하고,
세 table JOIN할수 있도록 변경하기
*/
